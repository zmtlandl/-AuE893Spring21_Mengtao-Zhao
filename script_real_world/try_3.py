...
def callback(data):
    ...
    front_wall = min(min(x[:35]),min(x[-35:]))
...
'''def camera_callback(data):
    ...
    if invoke_line_finder:
        ...
        if first_line_confirmation:
            linear = np.clip((line_find_factor * 0.05),0,0.08)
            angular = 0.2
            n = n + timestep*angular
    ...
    t1 = t0
    if a_temp == angular and b_temp == linear:
        line_find_factor = 0
        n = 0
    
    #control = [angular_zvel,linear_vel]
...
def tmnt_controller():
    ...
    image_sub = rospy.Subscriber("/camera/rgb/image_raw",LaserScan,camera_callback)          
    ...
    while not rospy.is_shutdown():
        ...
        if front_wall > 0.4 and front_wall < 7:
                ...
                if approach > 1: # Obstacle on the right
                    ...
                    if min(f_l_dist,f_r_dist)<Head_on_threshold: #imminent collision
                        ...
                        if -1*side_obstacle_confidence<delta<side_obstacle_confidence: #No danger of side collision
                            ...
                            delta = np.sign(delta) * 3.5 #side collision danger
            angular_zvel  = delta
            linear_vel   = 0.1
            ...
            #control = camera_callback()
            #linear_vel = linear
            #angular_zvel = angular
            LineFollower()
